...
class clear_self():
    def __init__(self, global_patch, t, show):
        self.patch = global_patch
    ...
    def RANSAC(self, index, step):
        ...
        y = max(y, 0)
        x = max(x, 0)
        self.patch[step][index+1] = self.patch[step][index+1][y:scale_h, x:scale_w]
    ...
    def generate(self, index, step):
        self.detectKeypoint()
        self.RANSAC(index, step)

    def getContainer(self):
        for i in xrange(9):
            if len(self.patch[i]):
                self.img1 = self.patch[i][0]
                self.img2 = self.patch[i][1]
                self.generate(0, i)
...
class patch():
    ...
    def getNext(self):
        if self.iter:
            ...
            cleaner = clear_self(self.global_patch, (self.tolerance_w, self.tolerance_h), 0)
